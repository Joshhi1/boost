...
def overview():
    print(f"\033[1;96m  ━━━━━━━━━━━━━━━━━━━[{red}UNO POGI{c}]━━━━━━━━━━━━━━━━━━━━━")
    total_accounts = count_lines("/sdcard/UNO/toka.txt")
    total_pages = count_lines("/sdcard/UNO/tokp.txt")
...
def ip():
    ...
    print(f"   {w}Tool Type{w}:{c}Free")
    print(f"   {w}IP{w}:{c}{ip_response.get('query', ' ')}")
    print(f"   {w}country{w}:{c}{ip_response.get('country', ' ')}")
    print(f"   {w}ISP{w}:{c}{ip_response.get('isp', ' ')}")
        ...
        if 1 <= choice <= len(txt_files):
            ...
            if os.path.isfile(selected_file):
                try:
                    ...
                    print(f'\033[1;31mError reading the file: {e}')
            else:
                print(f'\033[1;31mFile not found ')
        else:
...
def ManFile():
    ...
    if os.path.isfile(file):
        ...
        print(f' \033[1;31mFile location not found ')
...
def cuser(user, passw, user_choice):
    ...
    if "session_key" in pos:
        print(f"   {c}--> {user} extracted successfully.{wh}")
        ...
        if user_choice.lower() in ['n', 'no']:
            with open('/sdcard/UNO/tokpid.txt', 'a') as f:
                f.write(f'{c_user_value}\n')
            with open('/sdcard/UNO/tokp.txt', 'a') as f:
                f.write(f'{pos["access_token"]}\n')
        else:
            with open('/sdcard/UNO/toka.txt', 'a') as f:
                f.write(f'{pos["access_token"]}\n')
            with open('/sdcard/UNO/tokaid.txt', 'a') as f:
                f.write(f'{c_user_value}\n')
        
        with open('/sdcard/UNO/cok.txt', 'a') as f:
            f.write(f'{cookie}\n')
        
        with open('/sdcard/UNO/cokid.txt', 'a') as f:
            f.write(f'{c_user_value}\n')
    else:
...
def QuickReact():
    ...
    actor_ids = get_ids_tokens('/sdcard/UNO/tokaid.txt')
    tokens = get_ids_tokens('/sdcard/UNO/toka.txt')
...
def comment_on_facebook_post():
    user_ids = get_ids_tokens('/sdcard/UNO/tokaid.txt')
    access_tokens = get_ids_tokens('/sdcard/UNO/toka.txt')
    current_time = datetime.now()
    user_name = get_user_name()
    post_link = input('Enter the Facebook post link: ')
    post_id = linktradio(post_link)
